...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.relu(out)
        return out
# --- ResNet1D_Turbo: Yüksek Kapasiteli Model ---
class ResNet1D_Turbo(nn.Module):
    """
    Nano modelin 'abisi'. 
    Channels: 32 -> 64 -> 128 (Kapasite artırıldı)
    Amaç: %87 barajını aşıp %90+ görmek.
    """
    def __init__(self, num_classes=7, input_dim=39):
        super(ResNet1D_Turbo, self).__init__()
        
        # BAŞLANGIÇ KANAL SAYISINI ARTIRDIK (16 -> 32)
        self.inplanes = 32
        
        # Giriş Katmanı
        self.conv1 = nn.Conv1d(1, 32, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.GroupNorm(4, 32) # Yine GroupNorm ile stabilite koruyoruz
        self.relu = nn.ReLU(inplace=True)
        
        # KATMANLAR (32 -> 64 -> 128)
        # Nano'da hepsi 1 bloktu, burada da 1 tutuyoruz ama kanallar genişledi
        self.layer1 = self._make_layer(32, blocks=1, stride=1)
        self.layer2 = self._make_layer(64, blocks=1, stride=2)
        self.layer3 = self._make_layer(128, blocks=1, stride=2) # 128'e çıktık (En önemli fark)
        
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.dropout = nn.Dropout(0.5) # Kapasite arttığı için dropout'u 0.2 -> 0.3 yaptık
        
        # ÇIKIŞ (128 Kanal -> 7 Sınıf)
    ...
    def _make_layer(self, planes, blocks, stride=1):
        ...
        layers = []
        # ResidualBlock sınıfı yukarıda zaten tanımlı olduğu için aynen kullanıyoruz
...
# --- Factory Fonksiyonunu Güncelle ---
def create_ids_model(mode: str = "multiclass", num_classes: int = 7, input_dim: int = 39):
    print(f"[FACTORY] Initializing ResNet1D-Turbo (High Capacity Mode).")
    # ARTIK TURBO MODELİ ÇAĞIRIYORUZ
    model = ResNet1D_Turbo(num_classes=num_classes, input_dim=input_dim)
    return model